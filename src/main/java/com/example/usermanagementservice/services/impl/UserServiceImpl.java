...
import com.example.usermanagementservice.dtos.UserDetails;
...
private JwtUtils jwtUtils;
  ...
  public UserDetailsDto getUserProfile(String userEmail) {
    Optional<UserDetailsDto> userDetailsDto = userRepository.getUserDetails(userEmail);
    if(userDetailsDto.isEmpty()) {
      throw new UserDoesNotExistException("We couldn't find this user");
    }
    return userDetailsDto.get();
  }

@Override
public List<UserDetailsDto> findAllUsers(PageData pageData) {
    Long page = pageData.getPage();
    Long count = pageData.getCount();
    Long toSkip = (page - 1) * count;
    List<UserDetailsDto> allUsers = userRepository.getAllUsers(toSkip, count);
    return  allUsers;
}

@Override
public UserDetailsDto getAnotherUserProfile(String email) {
  Optional<UserDetailsDto> userDetailsDto = userRepository.getUserDetails(email);
  if(userDetailsDto.isEmpty()) {
    throw new UserDoesNotExistException("We couldn't find this user");
  }
  return userDetailsDto.get();
...
@Override
public int deleteMyProfile(String userEmail) {
  int deleteAction = userRepository.deleteMyAccount(userEmail);
